...
export class WSSEService {
  ...
  initValues() {
    // this.storage.get(APP_NAME+"_user")
    //     .then(data => {
    //       if (data) {
    //         console.log("VALUES : ", data);
    //         this.user_id  = data.user_id;
    //         this.salted_pwd = data.salted_password;
    //       }
    //     });
  }
  ...
  getHeaderValue() {
    ...
    let digest  = this.getDigest(nonce, created, this.salted_pwd);

    let header = "UsernameToken Username=\"" + this.user_id + "\", PasswordDigest=\"" + digest + "\", Nonce=\"" + nonce64 + "\", Created=\"" + created + "\"";
